...
function [gBest, gbestvalue, FEcount, etime, MaxFEs, Convergence_curve, iter] = mainePSO(Lmax1, level, Pxy, MaxFEs, Population_size, objectiveFunction, Iteration)
    % d = level-1;
    d = level;
    ...
    if MaxFEs == 0
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationPSO(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    else
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = EvolutionPSO(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    end
