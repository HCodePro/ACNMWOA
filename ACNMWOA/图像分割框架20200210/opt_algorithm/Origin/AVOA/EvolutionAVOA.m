function [Fbest, Lbest, FE, max_iter, convergence_curve, iter] = EvolutionAVOA(fname, pop_size, variables_no, lower_bound, upper_bound, max_iter, Pxy, Iteration)
    ...
    X = random_initialization(pop_size, dim, upper_bound, lower_bound);
    ...
    %%Main loop
    current_iter = 1; % Loop counter
    fes = 0;

    while fes < max_iter || current_iter <= Iteration

        for i = 1:size(X, 1)
            % Calculate the fitness of the population
            current_vulture_X = X(i, :);
            current_vulture_X = BC(current_vulture_X, lower_bound, upper_bound);
            [fitness, fes, current_vulture_X] = sigle_evaluation(current_vulture_X, dim, variables_no, fname, Pxy, fes);
            current_vulture_F = fitness;
        ...
        a = unifrnd(-2, 2, 1, 1) * ((sin((pi / 2) * (current_iter / Iteration)) ^ gamma) + cos((pi / 2) * (current_iter / Iteration)) - 1);
        P1 = (2 * rand + 1) * (1 - (current_iter / Iteration)) + a;
        ...
        if Fbest < Best_vulture1_F
            FE = fes;
            iter = current_iter;
        end

        Fbest = Best_vulture1_F;
        Lbest = Best_vulture1_X;
