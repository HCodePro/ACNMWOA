function [Fbest, Lbest, FE, Max_FEs, cg_curve, iter] = EvolutionLSHADE(fname, N, thdim, lb, ub, Max_FEs, Pxy, Iteration)
    ...
    for i = 1:size(pop, 1)
        pop(i, :) = BC(pop(i, :), lb, ub);
        [newfitness, FEs, pop(i, :)] = sigle_evaluation(pop(i, :), dim, thdim, fname, Pxy, FEs);
    ...
    while FEs <= Max_FEs || t <= Iteration
        ...
        for i = 1:size(ui, 1)
            ui(i, :) = BC(ui(i, :), lb, ub);
            [newfitness, FEs, ui(i, :)] = sigle_evaluation(ui(i, :), dim, thdim, fname, Pxy, FEs);
            children_fitness(i) = newfitness;
