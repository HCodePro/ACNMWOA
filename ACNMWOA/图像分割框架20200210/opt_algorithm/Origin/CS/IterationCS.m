...
function [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationCS(fname, SearchAgents_no, thdim, lb, ub, MaxFEs, Pxy, Iteration)
    dim = thdim / 2;
    Fbest = -inf;
    ...
    % end
    nest = random_initialization(n, dim, Ub, Lb);

    % Get the current best
    % fitness=10^10*ones(n,1);
    fitness = -inf * ones(n, 1);

    [fmin, bestnest, nest, fitness, N_iter] = get_best_nest(nest, nest, fitness, fname, Pxy, lb, ub, N_iter, dim, thdim);

    % N_iter=N_iter+n;
    Convergence_curve = [];

    %% Starting iterations
    while N_iter < MaxFEs || t <= Iteration

        % Generate new solutions (but keep the current best)
        new_nest = get_cuckoos(nest, bestnest, Lb, Ub);
        [fnew, best, nest, fitness, N_iter] = get_best_nest(nest, new_nest, fitness, fname, Pxy, lb, ub, N_iter, dim, thdim);
        % Update the counter
        %       N_iter=N_iter+n;
    ...
    function nest = get_cuckoos(nest, best, Lb, Ub)
        ...
        function [fmin, best, nest, fitness, FEs] = get_best_nest(nest, newnest, fitness, fname, Pxy, lb, ub, FEs, dim, thdim)
            % Evaluating all new solutions
            for j = 1:size(nest, 1)
                %     newnest(j,:)=simplebounds(newnest(j,:),Lb,Ub);
                Flag4ub = newnest(j, :) > ub;
                Flag4lb = newnest(j, :) < lb;
                newnest(j, :) = (newnest(j, :) .* (~(Flag4ub + Flag4lb))) + ub .* Flag4ub + lb .* Flag4lb;

                %     fnew=fobj(newnest(j,:));
                [fnew, FEs, newnest(j, :)] = sigle_evaluation(newnest(j, :), dim, thdim, fname, Pxy, FEs);
