...
function [gBest, gbestvalue, FEcount, etime, MaxFEs, Convergence_curve, iter] = maineACOR(Lmax1, level, Pxy, MaxFEs, Population_size, objectiveFunction, Iteration)
    ...
    if MaxFEs == 0
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationACOR(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    else
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = EvolutionACOR(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    end
