function [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationCBQMVO(fname, N, thdim, lb, ub, MaxFEs, Pxy, Iteration)
    dim = thdim / 2;
    ...
    while FEs < MaxFEs || Time <= Iteration
        ...
        %Eq. (3.3) in the paper
        WEP = WEP_Min + FEs * ((WEP_Max - WEP_Min) / Iteration);

        %Travelling Distance Rate (Formula): Eq. (3.4) in the paper
        TDR = 1 - ((FEs) ^ (1/6) / (Iteration) ^ (1/6));
