...
function [gBest, gbestvalue, FEcount, etime, MaxFEs, Convergence_curve, iter] = maineACNMWOA(Lmax1, level, Pxy, MaxFEs, Population_size, objectiveFunction, Iteration)
    % d = level-1;
    d = level;
    ...
    if MaxFEs == 0
        %% 迭代模式
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationACNMWOA(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    else
        %% 评估模式
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = EvolutionACNMWOA(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    end
