function [x, fval, exitflag, output, params] = fminsearchbnd(x0, LB, UB, options, dim, thdim, fname, Pxy, FEs, varargin)
    % size checks
    xsize = size(x0);
    x0 = x0(:);
    n = length(x0);
    ...
    if isempty(x0u)
        ...
        % stuff fval with the final value 计算适应度值
        %% Todo: 修改计算适应度值函数
        [fval, params.FEs, x] = sigle_evaluation(x, params.dim, params.thdim, params.fname, params.Pxy, params.FEs);
...
function fval = intrafun(x, params)
    ...
    % and call fun
    %% Todo: 修改计算适应度值函数
    x_1 = reshape(xtrans, params.xsize);
    [fval, params.FEs, x_1] = sigle_evaluation(x_1, params.dim, params.thdim, params.fname, params.Pxy, params.FEs);
