function [X, fitness, FEs] = CC(X, fitness, dim, lb, ub, FEs, dim_1, thdim, fname, Pxy)
    %%
    % Criss
    Mhc = zeros(size(X, 1), dim); %生成参考解空间
    Bhc = randperm(size(X, 1)); %生成1-size（X，1）打乱的序列
    ...
    for i = 1:(size(X, 1) / 2)
        ...
        for j = 1:dim
            ...
            c2 = (rand(1) * 2) - 1; %生成服从均匀分布的-1到1的随机数
    ...
    for i = 1:size(X, 1)
        ...
        % FEs = FEs + 1;
        [fitness_mhc(i), FEs, Mhc(i, :)] = sigle_evaluation(Mhc(i, :), dim_1, thdim, fname, Pxy, FEs);
    ...
    for i = 1:size(X, 1)
        ...
        % FEs = FEs + 1;
        [fitness_mvc(i), FEs, Mhc(i, :)] = sigle_evaluation(Mhc(i, :), dim_1, thdim, fname, Pxy, FEs);
