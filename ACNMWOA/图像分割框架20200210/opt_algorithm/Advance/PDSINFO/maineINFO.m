...
function [gBest, gbestvalue, FEcount, etime, MaxFEs, Convergence_curve, iter] = maineINFO(Lmax1, level, Pxy, MaxFEs, Population_size, objectiveFunction, Iteration)
    % d = level-1;
    d = level;
    ...
    if MaxFEs == 0
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationINFO(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    else
        [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = EvolutionINFO(objectiveFunction, Population_size, thdim, XVmin, XVmax, MaxFEs, Pxy, Iteration);
    end
