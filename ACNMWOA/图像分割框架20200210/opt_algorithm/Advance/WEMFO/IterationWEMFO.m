%MFO：飞蛾扑火优化算法
function [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationWEMFO(fname, N, thdim, lb, ub, MaxFEs, Pxy, Iteration)
    %输入：
    ...
    Convergence_curve = []; %zeros（a,b）生成全为0的a行b列矩阵
    it = 1; % Iteration 迭代
    ...
    while FEs < MaxFEs || it <= Iteration %判断是否到达最大评估次数
        Flame_no = round(N - it * ((N - 1) / Iteration)); % Flame_no 火焰数量 整数  是一个数字，不是数组
        ...
        w1 = (1 - it / Iteration) ^ (1 - tan(pi * (rand - 0.5)) * s / Iteration); %权重w会根据算法陷入局部最优程度呈曲线递减
        w2 = (2 - 2 * it / Iteration) ^ (1 - tan(pi * (rand - 0.5)) * s / Iteration); %权重w1会根据算法陷入局部最优程度呈曲线递减
        a1 = 2 - it * ((2) / Iteration); % a decreases linearly fron 2 to 0 in Eq. (2.3)
        a = -1 + it * ((-1) / Iteration);
