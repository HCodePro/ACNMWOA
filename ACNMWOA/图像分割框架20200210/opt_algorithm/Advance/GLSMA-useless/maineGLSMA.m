...
function [gBest,gbestvalue,FEcount,etime,MaxFEs,Convergence_curve,iter] = maineGLSMA(Lmax1,level,Pxy,MaxFEs,Population_size,objectiveFunction,Iteration)
% d = level-1;
d = level;
...
tic
if MaxFEs==0
	[Fbest,Lbest,FE,MaxFEs,Convergence_curve,iter]=IterationGLSMA(objectiveFunction,Population_size,thdim,XVmin,XVmax,MaxFEs,Pxy,Iteration);
else
	[Fbest,Lbest,FE,MaxFEs,Convergence_curve,iter]=EvolutionGLSMA(objectiveFunction,Population_size,thdim,XVmin,XVmax,MaxFEs,Pxy,Iteration);
end
toc
gBest=Lbest;
gbestvalue=Fbest;
