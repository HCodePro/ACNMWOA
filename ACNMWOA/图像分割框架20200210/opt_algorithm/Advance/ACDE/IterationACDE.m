...
function [Fbest, Lbest, FE, MaxFEs, Convergence_curve, iter] = IterationACDE(fname, SearchAgents_no, thdim, lb, ub, MaxFEs, Pxy, Iteration)
    % addpath([pwd,'\statics']);
    dim = thdim / 2;
    ...
    while FEs < MaxIt || it <= Iteration
        ...
        m = tanh(it, Iteration, [-2, 7]); % Eq (12)
        newPosition_memory = newPosition;
        newCost_memory = newCost;

        for i = 1:nPop
            Dimax = max(A_D(i, :));
            k = floor((1 - it / Iteration) * nPop) + 1; % Eq (9)
            ...
            if rand < p2
                ...
                if rand > 0.5 * it / Iteration + 0.25
                    u1 = rand(1, 2 * dim) > tr1;
                    ri = u1 .* random('Normal', zeros(1, 2 * dim), fr1 * (1 - it / Iteration) * (ub - lb)); % Eq (12)
                else
                    u2 = rand(1, 2 * dim) > tr2;
                    ...
                    % Eq (14)
                    if rand < 0.5
                        ri = fr2 * u2 .* (1 - it / Iteration) .* sin(2 * pi * rand(1, 2 * dim)) .* abs(rand(1, 2 * dim) .* Xw - newPosition_memory(i, :));
                    else
                        ri = fr2 * u2 .* (1 - it / Iteration) .* cos(2 * pi * rand(1, 2 * dim)) .* abs(rand(1, 2 * dim) .* Xw - newPosition_memory(i, :));
                    end
        ...
        for i = 1:nPop % 遍历每个个体
            ...
            [arfitness, arindex] = sort(arfitness, 'descend'); % max
