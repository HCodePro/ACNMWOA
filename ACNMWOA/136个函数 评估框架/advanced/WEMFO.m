%MFO：飞蛾扑火优化算法
function [Best_flame_pos, Convergence_curve] = WEMFO(N, MaxFEs, lb, ub, dim, fobj)
    %输入：
    ...
    Moth_pos = initialization(N, dim, ub, lb); %Initialize the positions of moths
    ...
    while FEs < MaxFEs %判断是否到达最大评估次数
        ...
        if it == 1 %如果迭代次数为1，Sort the first population of moths，计算飞蛾个体位置的适应度并排序，将排序后对应的个体位置存入火焰矩阵F并将飞蛾个体适应度赋值到火焰适应度矩阵；
            [fitness_sorted I] = sort(Moth_fitness); %sort  对矩阵进行升序排序
