function [Best_vulture1_X, convergence_curve] = AVOA(pop_size, max_iter, lower_bound, upper_bound, variables_no, fobj)
    ...
    while fes < max_iter
        ...
        a = unifrnd(-2, 2, 1, 1) * ((sin((pi / 2) * (fes / max_iter)) ^ gamma) + cos((pi / 2) * (fes / max_iter)) - 1);
        P1 = (2 * rand + 1) * (1 - (fes / max_iter)) + a;
