function [best_pos,Convergence_curve]=FOASR_GS_ChaoticD(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
tic
%% 果蝇的相关参数
...
%% 果蠅迭代尋優
 ...
 while FEs<MaxFEs     
    for i = 1:sizepop
        ...
        SChaos_ori = S_ori(i,:) + ChaosVec(FEs)* (S_ori(i,:) - best_pos(1,:));
        SChaos_fitness = fobj(SChaos);
        fitness_ori = fobj(S_ori(i,:));
        SChaos_fitness_ori = fobj(SChaos_ori);
        Moth_pos_m_gaus=S(i,:)*(1+randn(1));
        Moth_fitness_m_gaus=fobj(Moth_pos_m_gaus);
        Moth_fitness_s=fobj(S(i,:));
        Moth_fitness_comb=[Moth_fitness_m_gaus,SChaos_fitness,SChaos_fitness_ori,Moth_fitness_s,fitness_ori];
    ...
    if t<T
        if bestSmell<=Smell_avg
            X_axis = X_axis+X(bestindex,:);
            Y_axis = Y_axis+Y(bestindex,:);
        else
            X_axis = X_axis-X(worstindex,:);
            Y_axis = Y_axis-Y(worstindex,:);
        end
    else
        X_axis = X_axis*(1+randn(1));
        Y_axis = Y_axis*(1+randn(1));
    end
