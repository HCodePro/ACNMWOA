function [best_pos,Convergence_curve]=FOASR_GS_ChaoticB(SearchAgents_no,MaxFEs,lb,ub,dim,fobj)
tic
%% 果蝇的相关参数
...
%% 果蠅迭代尋優
 ...
 while FEs<MaxFEs   
    ...
    if t<T
        ...
        for k=1:sizepop
            P(k) = 4*(bestCVaccuarcy-Smell(i))/(bestCVaccuarcy-worstCVaccuarcy);
            if P(k) < Smell_avg
              X(k,:) = X(k,:)*(1+randn(1));
            end
        end
